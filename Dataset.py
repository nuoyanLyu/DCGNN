...
class Twibot22(Dataset):
    ...
    def Des_embbeding(self):
        ...
        if not os.path.exists(path):
            ...
            feature_extraction = pipeline('feature-extraction', model="distilroberta-base",
                                          tokenizer="distilroberta-base", device=0)
            des_vec = []
    ...
    def tweets_embedding(self):
        ...
        if not os.path.exists(path):
            ...
            print('Loading RoBerta')
            feature_extract = pipeline('feature-extraction', model='roberta-base', tokenizer='roberta-base', device=0,
...
class TwiBotBurstGraph(Dataset):
    def __init__(self, root='./Data/', device='cpu', inters=10):
        self.root = root
        self.device = device
        # inters indicates the time slice, such as 10 days, 20 days
        self.inters = inters
    ...
    def get_burst_graph(self):
        print('Building graph', end='   ')
        inters = str(self.inters)
        path0 = self.root + 'edgeList' + inters + 'Day'
        pathOrigin = path0 + 'Origin.pt'
        pathReply = path0 + 'Reply.pt'
        pathRetweet = path0 + 'Retweet.pt'
        pathQuote = path0 + 'Quote.pt'
        '''
                if not os.path.exists(path):
            id2index_dict = {id: index for index, id in enumerate(self.df_data['ID'])}
            edge_index = []
        ...
        # edge_index = torch.load(self.root + "edge_index.pt").to(self.device)
        # edge_type = torch.load(self.root + "edge_type.pt").to(self.device)
        edgeOrigin = torch.load(pathOrigin).to(self.device)
        edgeReply = torch.load(pathReply).to(self.device)
        edgeRetweet = torch.load(pathRetweet).to(self.device)
        edgeQuote = torch.load(pathQuote).to(self.device)
        print('Finished')
        # return edge_index, edge_type
        return edgeOrigin, edgeReply, edgeRetweet, edgeQuote
    ...
    def load_burst_num(self):
        print('loading burst num properties ...')
        inters = str(self.inters)
        # tweet_num_tensor = torch.load(self.root + 'numProperties' + inters + 'Day.pt').to(torch.float32)
        tweet_num_tensor = torch.load(self.root + 'num' + inters + 'day.pt').to(torch.float32)
        tweet_num_tensor.to(self.device)
        return tweet_num_tensor

    def load_burst_cat(self):
        print('loading burst cat properties ...')
        inters = str(self.inters)
        # burst_cat_tensor = torch.load(self.root + 'catProperties' + inters + 'Day.pt').to(torch.float32)
        burst_cat_tensor = torch.load(self.root + 'cat' + inters + 'Day.pt').to(torch.float32)
        burst_cat_tensor.to(self.device)
        return burst_cat_tensor

    def load_tweets_num(self):
        with open(self.root + 'userTweetNums' + str(self.inters) + 'Day.json') as f:
            tweet_num_list = json.load(f)
        return tweet_num_list

    def load_reindex_file(self):
        re_index = torch.load(self.root + 're_index.pt')
        return re_index

    def dataloader(self):
        labels = self.load_labels()
        # self.Des_Preprocess()
        # des_embedding进行的是用户描述的embedding过程，这里不再需要，删除
        # des_tensor = self.Des_embbeding()
        # self.tweets_preprocess()
        # tweets_tensor为推文embedding的结果，可以先删除？
        # tweets_tensor = self.tweets_embedding()

        # 加入新的文件
        burst_num_tensor = self.load_burst_num()
        burst_cat_tensor = self.load_burst_cat()
        tweet_range_list = self.load_tweets_num()
        re_index = self.load_reindex_file()
        ...
        # load新的图
        # edge_index, edge_type = self.Build_Graph()
        edgeOrigin, edgeReply, edgeRetweet, edgeQuote = self.get_burst_graph()

        # idx和label一样暂时不映射？
        train_idx, val_idx, test_idx = self.train_val_test_mask()
        # return des_tensor, tweets_tensor, num_prop, category_prop, edge_index, edge_type, labels, train_idx, val_idx, test_idx
        return burst_num_tensor, burst_cat_tensor, tweet_range_list,\
            edgeOrigin, edgeReply, edgeRetweet, edgeQuote, \
            labels, train_idx, val_idx, test_idx, re_index
